...
export default {
  get: () => fileList,
  post({ fileId, id, username, commentId, value }) {
    ...
    let idNumber
    for (let i = 0; i < basicfileList[fileId].commits.length; i++) {
      if (id === basicfileList[fileId].commits[i].id) {
        idNumber = i
      }
    }
    basicfileList[fileId].commits[idNumber].comments.push(comment)
    fileList = {
